...
class Trainer:
    def __init__(self, cfg, device="cuda"):
        ...
        '''
            ...
            还没有被拍扁
        '''
        train_dset = Dataset_MLG(cfg["exp"]["datadir"], cfg["train"]["n_rays"], "train", \
                                cfg["train"]["window_size"], cfg["train"]["window_num"], \
                                cfg["exp"]["image_init"], device) # 由dataset去构造数据集
        self.eval_dset = Dataset(cfg["exp"]["datadir"], cfg["train"]["n_rays"], "val", \
                                cfg["exp"]["image_init"], device) if self.i_eval > 0 else None
        ...
        self.img_init = train_dset.img_init
