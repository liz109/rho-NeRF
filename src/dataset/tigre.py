...
class TIGREDataset(Dataset):
    ...
    TIGRE dataset.
    """
    def __init__(self, path, n_rays=1024, type="train", mode="fdk", device="cuda"):    
        super().__init__()
        ...
        if type == "train":
            ...
            rays = self.get_rays(self.angles, self.geo, device)      # [50, 256, 256, 6] 在每一个角度下获取射线的原点和方向
            # stx()
            '''
            ...
            rays = self.get_rays(self.angles, self.geo, device)
            self.rays = torch.cat([rays, torch.ones_like(rays[...,:1])*self.near, torch.ones_like(rays[...,:1])*self.far], dim=-1)
