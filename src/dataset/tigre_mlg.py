...
class TIGREDataset_MLG(Dataset):
    ...
    TIGRE dataset.
    """
    def __init__(self, path, n_rays=1024, type="train", \
                 window_size = [32, 32], window_num = 4, \
                mode="fdk", device="cuda"):    
        super().__init__()
        ...
        self.img_init = torch.tensor(data["image_init"][mode]).to(device)
    ...
    def __getitem__(self, index):
        if self.type == "train":
            ...
            # 选取 window_inds
            select_inds_window = np.random.choice(projs_window_valid_indx.shape[0], size=[self.window_num], replace=False) # 从 0 ~ 64-1 中选取 window_num 个值
