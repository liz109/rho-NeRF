...
def config_parser():
    ...
    parser.add_argument("--config", default=f"./config/naf/chest_50.yaml", help="configs file path")
...
class BasicTrainer(Trainer):
    ...
    def compute_loss(self, data, global_step, idx_epoch):
        ...
        ret = render(rays, self.voxels, self.img_init,\
                     self.net, self.net_fine, **self.conf["render"])
        # stx()
        projs_pred = ret["acc"]
    ...
    def eval_step(self, global_step, idx_epoch):
        ...
        for i in tqdm(range(0, rays.shape[0], self.n_rays)):     # 每一簇射线是 n_rays ，每隔这么多射线渲染一次
            projs_pred.append(render(rays[i:i+self.n_rays], self.voxels, self.img_init, \
                                     self.net, self.net_fine, **self.conf["render"])["acc"])
        ...
        # rhos = matcher(self.eval_dset.voxels, self.eval_dset.voxels, self.img_init, mode="nearest")
        pts = torch.cat((pts, rhos), dim=-1)  
        image_pred = run_network(pts, self.net_fine if self.net_fine is not None else self.net, self.netchunk)
        image_pred = image_pred.squeeze()
        # stx()
        loss = {
            "proj_psnr": get_psnr(projs_pred, projs),
            "proj_ssim": get_ssim(projs_pred, projs),
            "psnr_3d": get_psnr_3d(image, image_pred),
            "ssim_3d": get_ssim_3d(image, image_pred),
            "ssim_3d_rhos": get_ssim_3d(rhos.squeeze(), image),
        }
        print("=========EVAL:", loss)
